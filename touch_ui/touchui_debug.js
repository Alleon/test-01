...
dhx.html={
	...
	insertBefore: function(node,before,rescue){
		if (!node) return;
		if (before && before.parentNode && before.parentNode.nodeType != 11)
...
dhx.protoUI({
	...
	render: function() {
        ...
        if(s.timeSelect){
			...
			selects[0].onmousedown = selects[1].onmousedown = function(e){
...
if(dhx.env.isFF && window.matchMedia){
	window.matchMedia("(orientation: portrait)").addListener(function() { dhx.ui.orientation = false});
	window.matchMedia("(orientation: landscape)").addListener(function() { dhx.ui.orientation = true});
}
